...
エンタープライズ級 OA 承認フロー管理システム（P2 完成版） | Spring Boot 3 + Vue 3 + MyBatis-Plus + JWT
...
│   │   ├── dto/                 # データ転送オブジェクト
│   │   ├── entity/              # エンティティクラス（7つ、ApprovalTask 含む）
...
│   └── src/
│       ├── api/                 # API ラッパー（auth / leave / template）
...
├── docs/
│   ├── mysql-p0-upgrade.sql     # データベース・テーブル作成 + シードデータ
│   ├── mysql-p1-upgrade.sql     # 承認ノード + フォームフィールド
...
| `approval_node` | 承認ノードテーブル（条件式、承認モード、タイムアウト設定をサポート） |
