...
## v3.0 Finance — Treasury（资金管理）📅 规划中
...
## v4.0.1 Stabilization（进行中）

**目的：** 在不动摇现有功能的前提下，加固安全、事务、模板版本化等基础设施。

### 已完成

| 阶段 | 内容 |
|------|------|
| Phase 1～3 | 安全配置、权限、资源级授权、JWT、HTTP 语义、真实数据库测试基础 |
| Phase 3.5 | Portfolio 管理员边界 |
| Phase 4A | 受限条件表达式执行器（替换早期任意 SpEL） |
| Phase 4B | 请求参数与 PathVariable 校验 |
| Phase 4C | 安全附件 URL 约束 |
| Phase 5A | 不可变模板版本 schema expand |
| Phase 5B-1 | DRAFT 生命周期与历史引用保护 |
| Phase 5B-2A | 行锁、事务串行化和真实 MySQL 并发证据 |

### 下一步

| 阶段 | 内容 |
|------|------|
| Phase 5B-2B | 发布、退休和状态转换事务 |
| Phase 5C | 请假/经费固定绑定具体模板版本 |
| Phase 5D | Controller/API 与前端过渡 |
| 最终 | Backfill / enforce / 最终约束收缩 |

---

## v5.0 Analytics（BI 分析）📅 规划中
